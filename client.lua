...
RegisterNetEvent('tmg-spawn:client:openUI', function(value)
...
RegisterNetEvent('tmg-houses:client:setHouseConfig', function(houseConfig)
    Houses = houseConfig
end)

RegisterNetEvent('tmg-spawn:client:setupSpawns', function(cData, new, apps)
    if not new then
        TMGCore.Functions.TriggerCallback('tmg-spawn:server:getOwnedHouses', function(houses)
            local myHouses = {}
            if houses ~= nil then
                for i = 1, (#houses), 1 do
                    myHouses[#myHouses + 1] = {
...
RegisterNUICallback('spawnplayer', function(data, cb)
    ...
    if type == 'current' then
        PreSpawnPlayer()
        TMGCore.Functions.GetPlayerData(function(pd)
            ped = PlayerPedId()
        ...
        if insideMeta.house ~= nil then
            ...
            TriggerEvent('tmg-houses:client:LastLocationHouse', houseId)
        elseif insideMeta.apartment.apartmentType ~= nil or insideMeta.apartment.apartmentId ~= nil then
            local apartmentType = insideMeta.apartment.apartmentType
            local apartmentId = insideMeta.apartment.apartmentId
            TriggerEvent('tmg-apartments:client:LastLocationHouse', apartmentType, apartmentId)
        end
        TriggerServerEvent('TMGCore:Server:OnPlayerLoaded')
        TriggerEvent('TMGCore:Client:OnPlayerLoaded')
        PostSpawnPlayer()
    elseif type == 'house' then
        PreSpawnPlayer()
        TriggerEvent('tmg-houses:client:enterOwnedHouse', location)
        TriggerServerEvent('TMGCore:Server:OnPlayerLoaded')
        TriggerEvent('TMGCore:Client:OnPlayerLoaded')
        TriggerServerEvent('tmg-houses:server:SetInsideMeta', 0, false)
        TriggerServerEvent('tmg-apartments:server:SetInsideMeta', 0, 0, false)
        PostSpawnPlayer()
    elseif type == 'normal' then
